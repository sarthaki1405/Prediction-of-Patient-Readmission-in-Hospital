# -*- coding: utf-8 -*-
"""OneHot_no_FeatureSelection.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vlR2u6LoCNatWFIulkL9h0eaIb3W8fqr
"""
...
def plot_confusion_matrix(model, x, y, cmap=plt.cm.Blues, normalize=None, ax=None):
    """
    ...
    Args:
        ...
        normalize: If set to 'true', normalize the confusion matrix.
        ax: Axis object to plot the confusion matrix.
    """
    y_pred = model.predict(x)
    cm = confusion_matrix(y, y_pred, normalize=normalize)
    classes = np.unique(y)

    if normalize == 'true' or normalize == True:  # Check for 'true' or True
    ...
    fmt = '.2f' if normalize == 'true' or normalize == True else 'd'  # Check for 'true' or True
    thresh = cm.max() / 2.
...
def compare_models(models, names, x, y):
    ...
    for i, model in enumerate(models):
        ...
        auc_roc = roc_auc_score(y, y_pred_prob, average='macro', multi_class='ovr')
        performance.append([names[i], accuracy, f1_micro, f1_macro, precision, recall, auc_roc])

    print(tabulate(performance, headers=["Model", "Accuracy", "F1 (micro)", "F1 (macro)", "Precision", "Recall", "AUC ROC"], tablefmt="pretty"))
...
rf_o = RandomForestClassifier(max_depth=10, random_state=42, class_weight={'<30': 1, '>30': 2, 'NO': 3})
...
xg_u.fit(X_train_undersampled, y_train_encoded)
evaluate_model(xg_u, X_test, y_test_encoded)
...
xg_o.fit(X_train_oversampled, y_train_oversampled_encoded)
evaluate_model(xg_o, X_test, y_test_encoded)
